...
class Translator
{
		...
		if (isset($options['translation_file_patterns']))
		{
			...
			foreach ($options['translation_file_patterns'] as $pattern)
			{
				...
				$translator->addTranslationFilePattern(
						$pattern['type'], $pattern['base_dir'], $pattern['pattern'], isset($pattern['text_domain']) ? $pattern['text_domain'] : 'default'
				);
			}
		}
		...
		if (isset($options['translation_files']))
		{
			...
			foreach ($options['translation_files'] as $file)
			{
				...
				$translator->addTranslationFile(
						$file['type'], $file['filename'], isset($file['text_domain']) ? $file['text_domain'] : 'default', isset($file['locale']) ? $file['locale'] : null
				);
			}
		}
		...
		if (isset($options['remote_translation']))
		{
			...
			foreach ($options['remote_translation'] as $remote)
			{
				...
				$translator->addRemoteTranslations(
						$remote['type'], isset($remote['text_domain']) ? $remote['text_domain'] : 'default'
				);
			}
		}
	...
	$singular, $plural, $number, $textDomain = 'default', $locale = null
	)
	{
		$locale = $locale ? : $this->getLocale();
		...
		if ($translation === null || $translation === '')
		{
			...
			)
			{
				return $this->translatePlural(
								$singular, $plural, $number, $textDomain, $fallbackLocale
				);
			}
	...
	$message, $locale, $textDomain = 'default'
	)
	{
		if ($message === '')
		{
		...
		if ($this->isEventManagerEnabled())
		{
			$results = $this->getEventManager()->trigger(
					self::EVENT_MISSING_TRANSLATION, $this, array(
				'message' => $message,
				'locale' => $locale,
	...
	$type, $filename, $textDomain = 'default', $locale = null
	)
	{
		$locale = $locale ? : '*';
	...
	$type, $baseDir, $pattern, $textDomain = 'default'
	)
	{
	...
	protected function loadMessages($textDomain, $locale)
	{
		...
		if (!$messagesLoaded)
		{
			...
			{
				$results = $this->getEventManager()->trigger(
						self::EVENT_NO_MESSAGES_LOADED, $this, array(
					'locale' => $locale,
					'text_domain' => $textDomain,
						), function ($r)
						{
